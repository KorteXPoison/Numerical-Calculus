...
def Jacobi(func_vec, x_vec = [], tolerancia = 0.0001, num_max_iter = 10):
	if not x_vec: x_vec = [0]*len(func_vec)

	diff = float('inf')
	while diff > tolerancia and num_max_iter:
		
		x_vec_new = [func_vec[i](x_vec) for i in range(len(x_vec))]

		diff = max([abs(x_vec_new[i]-x_vec[i]) for i in range(len(x_vec))])
...
func_vec = Jacobi_FunctionVec(matriz)

resultado = Jacobi(func_vec, x_vec, tolerancia, num_max_iteracoes)
